		...
		if (entries?.length) {
			for (const entry of entries) {
				this.addFields({
					...
					value: this.toUnorderedList(entry.weeklyWants.map((item) => `${item.rewardLevel} ${hyperlink(item.item?.name as string, databaseUrl(locale, ["item", item.item?.key as string]))}`)),
					inline: true,
				});
			}
		}
