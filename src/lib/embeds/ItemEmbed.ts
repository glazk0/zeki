			...
			for (const [key, value] of Object.entries(item.sources)) {
				if (key === 'stores') {
					...
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IRecipesItem[]).map((recipe) =>
								hyperlink(recipe.item?.name ?? 'Unknown', databaseUrl(locale, ['recipe', recipe.key as string]))
							)
						)
					});
					...
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IDialoguesItem[]).map((dialogue) =>
								hyperlink(dialogue.name ?? 'Unknown', databaseUrl(locale, ['dialogue', dialogue.key]))
							)
						)
					});
					...
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IMailMessagesItem[]).map((mail) =>
								hyperlink(mail?.name ?? 'Unknown', databaseUrl(locale, ['mail-messages', mail.key]))
							)
						)
					});
				} else if (key === 'books') {
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IBooksItem[]).map((book) =>
								hyperlink(book?.name ?? 'Unknown', databaseUrl(locale, ['item', book.key]))
							)
						)
					});
				} else if (key === 'quests') {
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IQuest[]).map((quest) => hyperlink(quest.name, databaseUrl(locale, ['quest', quest.key])))
						)
					});
				} else if (key === 'items') {
					this.addFields({
						...
						value: this.toUnorderedList(
							(value as IItemsItem[]).map((item) =>
								hyperlink(item.name ?? 'Unknown', databaseUrl(locale, ['item', item.key as string]))
							)
						)
					});
				}
