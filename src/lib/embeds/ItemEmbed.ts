				...
				if (key === "stores") {
					...
					this.addFields({
						...
						value: (value as IGatherablesItem[])
							.map(
								(gatherable) =>
									`- ${hyperlink(gatherable.name, `${PALIA_URL}/gatherable/${gatherable.key}`)} (${
										gatherable.skill && hyperlink(gatherable.skill?.name, `${PALIA_URL}/skill/${gatherable.skill?.key}`)
									})`,
							)
							.join("\n"),
					});
					...
					this.addFields({
						...
						value:
							(value as IMailMessagesItem[]).length > 5
								? (value as IMailMessagesItem[])
										.slice(0, 5)
										.map((mail) => `- ${hyperlink(mail?.name ?? "Unknown", `${PALIA_URL}/mail-messages/${mail.key}`)}`)
										.join("\n") + `\n...and ${value.length - 5} more`
								: (value as IMailMessagesItem[]).map((mail) => `- ${hyperlink(mail?.name ?? "Unknown", `${PALIA_URL}/mail-messages/${mail.key}`)}`).join("\n"),
					});
				} else if (key === "books") {
					this.addFields({
