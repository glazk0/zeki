...
export function registerClientEvents(): void {
  ...
  client.on(Events.ShardReconnecting, async (id: number) => {
    client.logger.warn(
      `Shard #${id} reconnecting for cluster #${client.cluster.id}`,
    );
  });

  client.on(Events.ShardResume, async (id: number, replayed: number) => {
    client.logger.info(
      `Shard #${id} resumed with ${replayed} replayed events for cluster #${client.cluster.id}`,
    );
  });

  client.on(Events.ShardError, async (error: any, id: number) => {
    client.logger.error(
      `Shard #${id} error for cluster #${client.cluster.id}`,
      error,
    );
  });
