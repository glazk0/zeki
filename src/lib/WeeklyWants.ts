...
// 		let exists = await this.client.cache.exists(`weekly_wants:${this.client.cluster.id}`);

// 		if (!exists) {
// 			await this.client.cache.set(`weekly_wants:${this.client.cluster.id}`, JSON.stringify(data));
// 			return;
// 		}

// 		let cache = (await this.client.cache.get(`weekly_wants:${this.client.cluster.id}`)) as IWeeklyWantsItem;
...
// 		settings = settings.filter((s) => clusterIdOfGuildId(this.client, s.guilds.guildId) === this.client.cluster.id);
...
// 			new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel("View full list").setURL(`${PALIA_URL}/tools/weekly-wants`),
// 			new ButtonBuilder().setURL(BOT_INVITE).setLabel("Invite me").setStyle(ButtonStyle.Link),
// 		);
